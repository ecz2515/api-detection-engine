...
class HeadersRequest(BaseModel):
    ...
    necessary_headers: Dict[str, str] = Field(
        ..., description="Minimal required headers"
    )
...
def test_api_with_headers(
    api_endpoint: str, method: str, headers: Dict[str, str]
) -> Dict[str, str]:
                ...
                try:
                    if method == "GET":
                        ...
                        data = (
                            ...
                            and "json" in headers["content-type"].lower()
                            else None
                        )
...
def format_endpoint_data(request, endpoint_data):
    ...
    return {
        ...
        if base_url in [r.api_endpoint for r in minimal_headers_data if r != request]
        else None,
    }
